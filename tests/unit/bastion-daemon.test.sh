...
got=$(env -u BACKUP_DEST -u CHECK_INTERVAL CONFIG_FILE="$WORK/bastion.conf" bash -c \
  'source services/bastion-daemon.sh; echo "$CHECK_INTERVAL|$AMBOSS_HEARTBEAT|$BACKUP_DEST"')
assert_eq "$got" "42|1|/mnt/backup_cln" "file values win; unset ones use the registry defaults"
